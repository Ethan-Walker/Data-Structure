...
MinHeap::MinHeap(int A[],int n)
{
    ...
    heap=new int[MaxHeapSize];                   //创建堆存储空间，并返回数组指针给堆指针
    for(int i=0;i<n;i++)
        heap[i]=A[i];                            //复制堆数组
...
bool MinHeap::Insert(const int& x)
{
    ...
    currentSize++;                                   //堆的长度+1
    siftUp(currentSize-1);                              //传入当前要调整的结点位置
...
void MinHeap::siftUp(int start)
{
    ...
    while(i>=0)
    {
        if(heap[i]<=heap[j])  break;                  //父结点值小，不调整,跳出循环
                ...
                t=heap[j];heap[j]=heap[i];heap[i]=t;  //插入结点小，与父结点交换
                cout<<left<<"调整"<<setw(3)<<t<<"结点 : ";
...
{
    int t,i,j;
    for(i=currentSize-1;i>=0;i--)                        //最小堆里面最终得到的是从大到小的顺序
    {
